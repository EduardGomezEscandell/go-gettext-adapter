...
// Adapted from github.com/ubuntu/adsys
...
func updatePo(potfile, localeDir string) error {
	...
	// Create pot file
	var files []string
	root, err := filepath.Abs(filepath.Dir(localeDir))
	if err != nil {
		return err
	}

	tmpRoot, err := sanitizegettext.SanitizeTemp(root, "i18n", "G")
	if err != nil {
		return fmt.Errorf("Failed to sanitize files: %v", err)
	}
	defer os.RemoveAll(tmpRoot)

	err = filepath.WalkDir(tmpRoot, func(p string, de fs.DirEntry, err error) error {
		if err != nil {
	...
	args := append([]string{
		...
		"-D", tmpRoot, "--output=" + potfile}, files...)
